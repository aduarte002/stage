...
public final class TrustedAPI extends ITrustedAPI.Stub {
    ...
    public synchronized IRemoteSharedPrefs openSharedPrefs(String packageName, String storeName,
        ...
        //Log.d(TAG, "openSharedPrefs(" + packageName + "/" + callerPackage + ")");
    ...
    @Override
    public synchronized void sharedPrefs ( final String store, final int value){
        Intent broadcastIntent = new Intent();
        broadcastIntent.setAction("OASIS.ACTION");

        broadcastIntent.putExtra(store, 1);
        mApplication.sendBroadcast(broadcastIntent);

    }
    @Override
    public synchronized void setStaticInt(final String store, final String var, final int newVar) {
        SharedPreferences prefs = mApplication.getApplicationContext().getSharedPreferences(store, Context.MODE_WORLD_WRITEABLE);
        Log.i("TAG","setStaticInt : "+ newVar);
        prefs.edit().putInt(var, newVar).commit();
